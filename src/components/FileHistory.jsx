...
async function fetchFileAtCommit(token, owner, repo, path, commitSha) {
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) return null // File didn't exist at this commit
...
// Fetch all commits in repo (for excavation)
async function fetchAllRepoCommits(token, owner, repo, branch = 'main', onProgress) {
  const commits = []
  let page = 1
  
  while (page <= 20) { // Up to 2000 commits
    try {
      onProgress?.({ page, status: `Loading page ${page}...` })
...
// Search through all commits for a term
async function searchInCommitHistory(token, owner, repo, query, branch = 'main', onProgress) {
  ...
  onProgress?.({ status: `Found ${results.commitMessages.length} matching commits` })
...
async function fetchPRsForCommits(token, owner, repo, commitShas) {
  ...
  // Use search API to find PRs
  for (const sha of commitShas.slice(0, 10)) { // Limit to prevent rate limits
    try {
      const response = await fetch(
...
function CommitCard({ commit, org, repo, isSelected, onSelect, showCheckbox = false }) {
  const commitUrl = commit.url || `https://github.com/${org}/${repo}/commit/${commit.sha}`
  
  return (
    <div 
      className={`bg-void-700/30 border rounded-xl p-4 transition-all ${
        isSelected ? 'border-electric-400/50 bg-electric-400/5' : 'border-void-600/50 hover:border-frost-300/30'
      }`}
    >
      <div className="flex items-start gap-3">
        {showCheckbox && (
          <button 
            ...
            className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-1 ${
              isSelected ? 'bg-electric-400 border-electric-400' : 'border-frost-300/30 hover:border-frost-300/50'
            }`}
          >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-frost-100 text-sm line-clamp-2">{commit.message}</p>
...
export function FileHistory({ token, org, allRepos }) {
  ...
  const handleExcavation = async (e) => {
    e?.preventDefault()
    if (!excavationQuery.trim() || !selectedRepo || !selectedBranch) return
    ...
    try {
      const results = await searchInCommitHistory(
        token, 
        org, 
        selectedRepo.name, 
        excavationQuery, 
        selectedBranch,
        setExcavationProgress
      )
      
      setExcavationResults(results)
      
      if (results.commitMessages.length > 0) {
        toast.success(`Found ${results.commitMessages.length} matching commits`)
        ...
        toast.info('No matching commits found')
      }
    } catch (e) {
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-void-700/50 border border-void-600/50 rounded-xl text-frost-100 placeholder-frost-300/40 focus:outline-none focus:border-orange-400/50 transition-all" 
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-y-auto">
              {filteredRepos.map(repo => (
                <button 
                  ...
                  className="flex items-center gap-3 p-3 rounded-xl border bg-void-700/30 border-void-600/50 hover:border-orange-400/50 hover:bg-orange-400/5 transition-all text-left"
                >
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <select 
                ...
                className="px-3 py-2 bg-void-700/50 border border-void-600/50 rounded-lg text-frost-100 text-sm focus:outline-none focus:border-orange-400/50"
              >
                {branches.map(b => (
              ...
              <div className="flex items-center gap-2 ml-auto">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${mode === 'file' ? 'bg-orange-500/20 text-orange-400 border border-orange-400/30' : 'bg-void-700/30 text-frost-300/60'}`}
                >
                  <FileText className="w-4 h-4" /> File History
                </button>
                <button
                  onClick={() => setMode('excavation')}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${mode === 'excavation' ? 'bg-purple-500/20 text-purple-400 border border-purple-400/30' : 'bg-void-700/30 text-frost-300/60'}`}
                >
          ...
          <div className="bg-void-700/30 border border-void-600/50 rounded-2xl p-4 lg:col-span-1">
            ...
            {loadingTree ? (
              ...
              <div className="max-h-[500px] overflow-auto">
                <FileTree 
                  tree={tree} 
                  selectedPaths={selectedFile ? new Set([selectedFile]) : new Set()} 
          ...
          <div className="lg:col-span-2 space-y-4">
            {!selectedFile ? (
              <div className="bg-void-700/30 border border-void-600/50 rounded-2xl p-8 text-center">
                ...
                <p className="text-xs text-frost-300/40 mt-2">Including all versions, even deleted content</p>
              </div>
            ) : (
              <>
                {/* Selected File Info */}
                ...
                {loadingFileHistory ? (
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-sm text-frost-300/60 font-medium flex items-center gap-2">
                    ...
                    <div className="bg-void-900/50 border border-void-600/50 rounded-xl overflow-hidden">
                      <div className="px-4 py-3 border-b border-void-600/30 flex items-center justify-between">
                        <span className="text-sm text-frost-300/60">
                          {viewingCommit ? `Version @ ${viewingCommit.substring(0, 7)}` : 'Select a version'}
                        </span>
                        {viewingCommit && (
                          <a 
                            href={`https://github.com/${org}/${selectedRepo.name}/blob/${viewingCommit}/${selectedFile}`}
                            target="_blank"
                            rel="noopener noreferrer"
                      ...
                      <div className="max-h-[400px] overflow-auto">
                        {loadingContent ? (
                          ...
                          <pre className="p-4 text-xs font-mono text-frost-300/80 whitespace-pre-wrap">
                          ...
                          <div className="p-8 text-center">
                            ...
                            <p className="text-xs text-frost-300/40 mt-1">It was added in a later commit</p>
                          </div>
                        ) : (
                          <div className="p-8 text-center text-frost-300/40">
          ...
          <div className="bg-void-700/30 border border-void-600/50 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-frost-300/60">Search through ALL commits ever made - find deleted code, old implementations, history</p>
              </div>
            </div>
            ...
            <form onSubmit={handleExcavation} className="flex gap-2">
              <input 
                ...
                placeholder="Search commit messages, code that once existed..." 
            ...
            {excavationProgress && (
              <div className="mt-4 flex items-center gap-3 text-sm text-frost-300/60">
          ...
          {excavationResults && (
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {associatedPRs.map(pr => (
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 p-3 bg-void-600/30 rounded-xl hover:bg-void-600/50 transition-colors"
                      >
                        {pr.state === 'merged' ? (
              ...
              <div className="bg-void-700/30 border border-void-600/50 rounded-2xl p-6">
                <h4 className="text-frost-100 font-medium mb-4 flex items-center gap-2">
                  <GitCommit className="w-4 h-4 text-neon-green" />
                  Matching Commits ({excavationResults.commitMessages.length})
                </h4>
