...
function CodePreview({ result, token, org, selectedBranches }) {
  ...
  const loadContent = async () => {
    ...
    setLoading(true)
    try {
      const fileContent = await getFileContent(token, org, result.repository.name, result.path, branch)
    ...
    <div className="bg-void-700/30 border border-void-600/50 rounded-xl overflow-hidden hover:border-electric-400/30 transition-all group">
      <div className="p-4">
        ...
        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-void-600/30">
          ...
          <a href={`https://github.com/${org}/${result.repository.name}/commits/${branch}/${result.path}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-frost-300/60 hover:text-frost-200 transition-colors">
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-4">
      <button
        ...
        className="p-2 rounded-lg bg-void-700/50 hover:bg-void-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      >
          ...
          <button
            ...
            className={`w-8 h-8 rounded-lg text-sm font-medium transition-all ${
              currentPage === i 
                ...
                : 'bg-void-700/50 text-frost-300/60 hover:bg-void-700 hover:text-frost-200'
            }`}
          >
            {i + 1}
          </button>
  ...
  return (
    <div className="bg-void-700/30 border border-void-600/50 rounded-xl p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-frost-300/60 px-2 py-0.5 bg-void-600/50 rounded">
      ...
      <button
        onClick={onSelectAll}
        className="text-xs text-electric-400 hover:text-electric-500 transition-colors mb-3 text-left"
      >
            ...
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all text-left ${
                selectedBranches.has(branch.name)
                  ...
                  : 'bg-void-600/30 text-frost-300/60 border border-transparent hover:border-frost-300/20'
              }`}
            >
  ...
  return (
    <div className="bg-void-700/30 border border-void-600/50 rounded-xl p-4 h-full flex flex-col">
      <div className="flex items-center gap-2 mb-3">
        <Folder className="w-5 h-5 text-yellow-400" />
        <span className="font-medium text-frost-100 truncate">{repo.name}</span>
        <span className="text-xs text-frost-300/60 px-2 py-0.5 bg-void-600/50 rounded ml-auto">
...
export function CodeSearch({ token, org, allRepos }) {
  ...
  const handleSelectAllBranchesForRepo = (repoName) => {
    const branches = (branchesMap[repoName] || []).filter(b => 
      b.name.toLowerCase().includes(branchSearch.toLowerCase())
    )
  ...
  const handleConfirmBranches = async () => {
    ...
    // Initialize selected paths map
    const newSelectedPathsMap = {}
    selectedRepos.forEach(repo => {
      newSelectedPathsMap[repo.name] = new Set()
    })
    setSelectedPathsMap(newSelectedPathsMap)
    
    // Load trees for each repo
    const newLoadingTrees = {}
    selectedRepos.forEach(repo => {
      newLoadingTrees[repo.name] = true
    })
      ...
      <div className="bg-void-700/30 border border-void-600/50 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-display font-semibold text-frost-100">Code Search</h2>
              <p className="text-sm text-frost-300/60">Multi-repo, multi-branch search</p>
            </div>
          </div>
          {step > 1 && (
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${
                      isSelected ? 'bg-electric-400/10 border-electric-400/50' : 'bg-void-700/30 border-void-600/50 hover:border-frost-300/30'
                    }`}
                  >
        ...
        {/* Step 2: Branch Selection - Separate boxes per repo */}
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search branches across all repos..."
                className="w-full pl-10 pr-4 py-2.5 bg-void-700/50 border border-void-600/50 rounded-xl text-frost-100 placeholder-frost-300/40 focus:outline-none focus:border-electric-400/50 transition-all"
              />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {branchPageRepos.map(repo => (
                    <RepoBranchCard
                      ...
                      onToggleBranch={(branchName) => handleToggleBranch(repo.name, branchName)}
        ...
        {/* Step 3: File Selection - Separate boxes per repo */}
        {step === 3 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-frost-300/60">Select files/folders for each repo (optional)</span>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setSelectedPathsMap(prev => {
                  const cleared = {}
                  selectedRepos.forEach(r => cleared[r.name] = new Set())
                  return cleared
                })}
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <input type="text" value={extension} onChange={(e) => setExtension(e.target.value)} placeholder="Extension (e.g., tsx)" className="px-3 py-2 bg-void-700/50 border border-void-600/50 rounded-lg text-frost-100 placeholder-frost-300/40 text-sm focus:outline-none focus:border-electric-400/50 w-40" />
              </div>
            </form>
          </div>
        )}
          ...
          {results.items.length === 0 ? (
            ...
            <div className="space-y-3">
              {results.items.map((result, index) => (
                <CodePreview key={`${result.repository.fullName}-${result.path}-${index}`} result={result} token={token} org={org} selectedBranches={result.searchedBranches || []} />
              ))}
            </div>
          )}
        </div>
